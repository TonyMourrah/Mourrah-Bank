...
export default function Dashboard() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setFormError(
        err.response?.data || "Erreur lors de la création de l'enveloppe.",
      );
    } finally {
      setCreating(false);
    }
  ...
  const totalCategories = enveloppesCategories.reduce(
    (sum, e) => sum + e.montant,
    0,
  );
  ...
  const renderProgress = (enveloppe) => {
    ...
    const pct = Math.max(
      0,
      Math.min(100, (enveloppe.montant / enveloppe.limite) * 100),
    );
      ...
      <div className="mt-2">
        <div className="d-flex justify-content-between mb-1">
          <small className="text-muted">
            {label} : {enveloppe.limite.toFixed(2)} $
          </small>
          <small className={depasse ? "text-danger fw-bold" : "text-muted"}>
            {pct.toFixed(0)}%{" "}
            {enveloppe.type === "EPARGNE" ? "épargné" : "dépensé"}
          </small>
        </div>
        ...
        {enveloppe.type === "EPARGNE" && pct >= 100 && (
          <small className="text-success">
            <i className="bi bi-check-circle-fill me-1"></i>Objectif atteint !
          </small>
        )}
        {enveloppe.type === "BUDGET" && depasse && (
          <small className="text-danger">
            <i className="bi bi-exclamation-triangle-fill me-1"></i>Limite
            dépassée
          </small>
        )}
      </div>
  ...
  return (
    <div>
      ...
      <div className="container mt-4">
          ...
          <div className="row g-3 mb-4 fade-in">
            <div className="col-md-4">
              ...
              >
                <div className="d-flex align-items-center gap-3">
                  <i
                    className="bi bi-wallet2 text-white"
            ...
            <div className="col-md-4">
              <div className="card border-0 shadow-sm card-hover p-3">
                <div className="d-flex align-items-center gap-3">
                  <i
                    className="bi bi-envelope-paper text-primary"
                    style={{ fontSize: "1.8rem" }}
                  ></i>
                  <div>
                    <small className="text-muted">Alloué en catégories</small>
                    <h4 className="text-primary fw-bold mb-0">
                      {totalCategories.toFixed(2)} $
                    </h4>
                  </div>
                </div>
              </div>
            ...
            <div className="col-md-4">
              <div className="card border-0 shadow-sm card-hover p-3">
                <div className="d-flex align-items-center gap-3">
                  <i
                    className="bi bi-piggy-bank text-success"
                    style={{ fontSize: "1.8rem" }}
                  ></i>
                  <div>
                    <small className="text-muted">Non alloué</small>
                    <h4 className="text-success fw-bold mb-0">
                      {nonAlloue ? nonAlloue.montant.toFixed(2) : "0.00"} $
                    </h4>
                  </div>
                </div>
              </div>
        ...
        {showForm && (
          <div
            className="card p-4 shadow-sm border-0 card-hover fade-in mb-4"
                ...
                <div className="d-flex gap-3">
                  <div className="form-check">
                    <input
                      ...
                      id="typeBudget"
                      name="type"
                      checked={newType === "BUDGET"}
                      onChange={() => setNewType("BUDGET")}
                    />
                    <label className="form-check-label" htmlFor="typeBudget">
                      <i className="bi bi-cart me-1"></i>Budget de dépenses
                    </label>
                  </div>
                  <div className="form-check">
                    <input
                      ...
                      id="typeEpargne"
                      name="type"
                      checked={newType === "EPARGNE"}
                      onChange={() => setNewType("EPARGNE")}
                    />
                    <label className="form-check-label" htmlFor="typeEpargne">
                      <i className="bi bi-piggy-bank me-1"></i>Épargne
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-3">
                ...
                {newType === "BUDGET" && (
                  <div className="form-text">
                    Ce montant augmente à mesure que tu réalloues (dépenses)
                    de l'argent vers cette enveloppe.
                  </div>
                )}
              </div>
              <div className="mb-3">
        ...
        {nonAlloue && (
          <div className="mb-4">
            <h6 className="text-muted mb-2 fade-in">
              <i className="bi bi-piggy-bank me-1"></i>Réservoir non alloué
            </h6>
            <div
              className="card shadow-sm card-hover fade-in position-relative"
              style={{ maxWidth: "340px", border: "2px dashed #28a745" }}
            >
              <button
        ...
        {!loading && enveloppesCategories.length === 0 && !error && (
          <div className="text-center py-5 text-muted fade-in">
            ...
            <p className="mt-2">
              Aucune catégorie pour l'instant. Crée-en une pour commencer.
            </p>
          </div>
        )}
        ...
        <div className="row g-3">
          {enveloppesCategories.map((enveloppe, i) => (
            <div className="col-md-4" key={enveloppe.id}>
              <div
                className="card shadow-sm border-0 card-hover fade-in position-relative"
              ...
              >
                ...
                <div className="card-body">
                  <div className="d-flex justify-content-between align-items-start mb-2">
                    ...
                    <i
                      className={`bi ${enveloppe.type === "EPARGNE" ? "bi-piggy-bank" : "bi-envelope-fill"} text-primary`}
                    ></i>
                  </div>
                  <h3 className="text-primary fw-bold">
                    {enveloppe.montant.toFixed(2)} $
                  </h3>
                  <small className="text-muted">
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSaveEdit}>
                  ...
                  <div className="d-flex gap-3">
                    <div className="form-check">
                      <input
                        ...
                        id="editTypeBudget"
                        name="editType"
                        checked={editType === "BUDGET"}
                        onChange={() => setEditType("BUDGET")}
                      />
                      <label
                        className="form-check-label"
                        htmlFor="editTypeBudget"
                      >
                        Budget
                      </label>
                    </div>
                    <div className="form-check">
                      <input
                        ...
                        id="editTypeEpargne"
                        name="editType"
                        checked={editType === "EPARGNE"}
                        onChange={() => setEditType("EPARGNE")}
                      />
                      <label
                        className="form-check-label"
                        htmlFor="editTypeEpargne"
                      >
                        Épargne
                      </label>
                    </div>
              ...
              <div className="mb-3">
                <label className="form-label">
                  {!editing.nonAlloue && editType === "BUDGET"
                    ? "Montant déjà dépensé"
                    : "Montant"}
                </label>
                <input
